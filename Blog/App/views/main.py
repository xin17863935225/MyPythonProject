...
# def show(page):
#     print('你能看到我几次?')
#     pagination = Posts.query.filter_by(pid=0).order_by(Posts.timestamp.desc()
...
def index():
    ...
    if not value:
        print('没走缓存...')
        pagination = Posts.query.filter_by(pid=0,state=True).order_by(Posts.timestamp.desc()
            ).paginate(page,current_app.config['PAGE_NUM'],False)
        data = pagination.items  # 获取当前页面的数据
...
def search():
    ...
    pagination = Posts.query.filter(or_(Posts.title.contains(word),Posts.content.contains(word)
        ),and_(Posts.pid==0,Posts.state==True)
